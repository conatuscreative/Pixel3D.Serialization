...
namespace Pixel3D.Serialization.Discovery
{
    class TypeClassifier
    {
        ...
        public HashSet<Type> ValueTypes { get { return typeDiscovery.valueTypes; } }
        public HashSet<Type> ReferenceTypes { get { return typeDiscovery.referenceTypes; } }
        ...
        public void RunClassification()
        {
            ...
            {
                if(fieldDispatch.Value.Count > 1) // More than one type that we can dispatch to (otherwise will do direct dispatch)
                {
        ...
        public void WriteReport(StreamWriter report, StreamWriter errors)
        {
            ...
            foreach(var type in noDispatch)
            {
                ...
                if(type.IsInterface)
                    ...
                    errors.WriteLine("  Fields will be serialized directly as " + type + " (even if that is not the true type), and cannot be deserialized");
                errors.WriteLine();
            }
            report.WriteLine();
