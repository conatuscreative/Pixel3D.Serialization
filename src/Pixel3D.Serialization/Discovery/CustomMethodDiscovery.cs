...
namespace Pixel3D.Serialization.Discovery
{
    ...
    {
        public static SerializationMethodProviders Run(IEnumerable<Assembly> assemblies, StreamWriter report, StreamWriter errors)
        {
            LookupMethodProvider customValueTypeSerializeMethods        = new LookupMethodProvider();
            LookupMethodProvider customValueTypeDeserializeMethods      = new LookupMethodProvider();
            LookupMethodProvider customReferenceTypeSerializeMethods    = new LookupMethodProvider();
            LookupMethodProvider customReferenceTypeDeserializeMethods  = new LookupMethodProvider();
            LookupMethodProvider customReferenceFieldSerializeMethods   = new LookupMethodProvider();
            LookupMethodProvider customReferenceFieldDeserializeMethods = new LookupMethodProvider();
            LookupMethodProvider customReferenceTypeInitializerMethods  = new LookupMethodProvider();

            BindingFlags bindingFlags = BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static;
            if(errors != null)
                bindingFlags |= BindingFlags.Instance; // If we're reporting errors, also check instance methods to report them as errors (otherwise skip them for speed)

            var allTypes = assemblies.SelectMany(a => a.GetTypes());
            var allMethods = allTypes.SelectMany(t => t.GetMethods(bindingFlags));

            var customSerializerCandidates = allMethods.Where(m => m.GetCustomAttributes(typeof(CustomSerializerAttribute), false).Length > 0);
            var customInitializerCandidates = allMethods.Where(m => m.GetCustomAttributes(typeof(CustomInitializerAttribute), false).Length > 0);
            var customFieldSerializerCandidates = allMethods.Where(m => m.GetCustomAttributes(typeof(CustomFieldSerializerAttribute), false).Length > 0);

            #region Handle [CustomSerializer]

            Type[] customSerializerParameters = new[] { typeof(SerializeContext), typeof(BinaryWriter), null };
            Type[] customDeserializerParameters = new[] { typeof(DeserializeContext), typeof(BinaryReader), null };

            foreach(var method in customSerializerCandidates)
            {
                bool serialize = MatchStaticSignature(method, typeof(void), customSerializerParameters);
                bool deserialize = MatchStaticSignature(method, typeof(void), customDeserializerParameters);
                ...
                Type type = method.GetParameters()[2].ParameterType;

                if(!ValidateGenericTypeUsage(method, type, errors, "[CustomSerializer]"))
                    continue;

                bool originalByRef = type.IsByRef;
                if(originalByRef)
                ...
                if(type.IsInterface)
                {
                    if(errors != null)
                    {
                        errors.WriteLine("ERROR: [CustomSerializer] method is not valid for interfaces: " + FormatMethodName(method));
                ...
                if(type.IsValueType)
                    ...
                    methodTable = serialize ? customReferenceTypeSerializeMethods : customReferenceTypeDeserializeMethods;
            ...
            foreach(var method in customInitializerCandidates)
            {
                ...
                if(type.IsValueType)
                {
                    if(errors != null)
                    {
                        errors.WriteLine("ERROR: [CustomInitializer] is not possible for value types: " + FormatMethodName(method));
                        errors.WriteLine();
                    }
                    continue;
                }
                ...
                if(type.IsInterface)
                {
                    if(errors != null)
                    {
                        errors.WriteLine("ERROR: [CustomInitializer] method is not valid for interfaces: " + FormatMethodName(method));
                        errors.WriteLine();
                    }
                    continue;
                }
            ...
            foreach(var method in customFieldSerializerCandidates)
            {
                ...
                if(type.IsValueType)
                {
                    if(errors != null)
                    {
                        errors.WriteLine("ERROR: [CustomFieldSerializer] cannot be applied to value types (Use a [CustomSerializer] instead): " + FormatMethodName(method));
                        errors.WriteLine("  " + method);
                        errors.WriteLine();
                    }
                    continue;
                ...
                LookupMethodProvider methodTable = serialize ? customReferenceFieldSerializeMethods : customReferenceFieldDeserializeMethods;
            ...
            DoMatchCheck(customReferenceTypeSerializeMethods.lookup, customReferenceTypeDeserializeMethods.lookup, errors);
            DoMatchCheck(customReferenceFieldSerializeMethods.lookup, customReferenceFieldDeserializeMethods.lookup, errors);

            if(report != null)
            {
                DoReport(report, "Value Types", customValueTypeSerializeMethods.lookup, customValueTypeDeserializeMethods.lookup);
                DoReport(report, "Reference Types", customReferenceTypeSerializeMethods.lookup, customReferenceTypeDeserializeMethods.lookup, customReferenceTypeInitializerMethods.lookup);
                DoReport(report, "Reference Fields", customReferenceFieldSerializeMethods.lookup, customReferenceFieldDeserializeMethods.lookup);
            }
        ...
        private static bool ValidateGenericTypeUsage(MethodInfo method, Type subjectType, StreamWriter errors, string source)
        {
            if(subjectType.ContainsGenericParameters && (subjectType.IsArray || (subjectType.IsByRef && subjectType.GetElementType().IsArray)))
            {
                if(errors != null)
                {
                    errors.WriteLine("ERROR: " + source + " generic array type not supported in: " + FormatMethodName(method));
                    errors.WriteLine("  " + method);
                    errors.WriteLine();
                }
                return false;
            ...
            if(method.ContainsGenericParameters && (!subjectType.ContainsGenericParameters || !method.GetGenericArguments().SequenceEqual(subjectType.GetGenericArguments())))
            {
                if(errors != null)
                {
                    errors.WriteLine("ERROR: " + source + " method's generic arguments do not match the subject's generic arguments: " + FormatMethodName(method));
                    errors.WriteLine("  " + method);
                    errors.WriteLine();
                }
                return false;
        ...
        private static bool MatchStaticSignature(MethodInfo method, Type requestedReturnType, Type[] requestedParameterTypes)
        {
            if(!method.IsStatic)
        ...
        private static void DoMatchCheck(Dictionary<Type, MethodInfo> customSerializeMethods, Dictionary<Type, MethodInfo> customDeserializeMethods, StreamWriter errors)
        {
            List<Type> notMatchingTypes = new List<Type>();
                ...
                Dictionary<Type, MethodInfo> customInitializerMethods = null)
        {
            ...
            HashSet<Type> allCustomTypes = new HashSet<Type>();
        ...
        public static void CheckForDerivedCustomInitializers(MethodProvider initializerMethods, HashSet<Type> referenceTypes, StreamWriter errors)
        {
            foreach(var type in referenceTypes)
            {
                ...
                while(baseType != null && baseType != typeof(object))
                {
                    ...
                    {
                        errors.WriteLine("ERROR: Type " + type + " is drived from type " + baseType + " with a custom initializer!");
