...
namespace Pixel3D.Serialization
{
    internal class SerializationMethodProviders
    {
        ...
        {
            this.ValueTypeSerializeMethods        = valueTypeSerializeMethods;
            this.ValueTypeDeserializeMethods      = valueTypeDeserializeMethods;
            this.ReferenceTypeSerializeMethods    = referenceTypeSerializeMethods;
            this.ReferenceTypeDeserializeMethods  = referenceTypeDeserializeMethods;
            this.ReferenceFieldSerializeMethods   = referenceFieldSerializeMethods;
            this.ReferenceFieldDeserializeMethods = referenceFieldDeserializeMethods;
            this.ReferenceTypeInitializeMethods   = referenceTypeInitializeMethods;
        }


        public static SerializationMethodProviders Combine(SerializationMethodProviders first, SerializationMethodProviders second)
        {
            return new SerializationMethodProviders(
                    ...
                    FallbackMethodProvider.Combine(first.ReferenceTypeSerializeMethods   , second.ReferenceTypeSerializeMethods   ),
                    FallbackMethodProvider.Combine(first.ReferenceTypeDeserializeMethods , second.ReferenceTypeDeserializeMethods ),
                    FallbackMethodProvider.Combine(first.ReferenceFieldSerializeMethods  , second.ReferenceFieldSerializeMethods  ),
                    FallbackMethodProvider.Combine(first.ReferenceFieldDeserializeMethods, second.ReferenceFieldDeserializeMethods),
                    FallbackMethodProvider.Combine(first.ReferenceTypeInitializeMethods  , second.ReferenceTypeInitializeMethods  ));
        }
        ...
        static Delegate CreateDelegate(Type delegateType, MethodInfo method)
        {
        ...
        public FieldSerializeMethod<T> GetFieldSerializeDelegate<T>()
        {
            ...
            else
            {
                ...
                var serializeField = (Action<SerializeContext, BinaryWriter, T>)CreateDelegate(typeof(Action<SerializeContext, BinaryWriter, T>), method);

                FieldSerializeMethod<T> wrapper = (SerializeContext context, BinaryWriter bw, ref T obj) => serializeField(context, bw, obj);
                return wrapper;
            }
        }
        ...
        public bool HasTypeSerializer(Type type)
        {
            if(type.IsValueType)
            {
                bool hasSerializeMethod = (ValueTypeSerializeMethods.GetMethodForType(type) != null);
                Debug.Assert(hasSerializeMethod == (ValueTypeDeserializeMethods.GetMethodForType(type) != null)); // <- debug check for matching deserializer
                return hasSerializeMethod;
            }
            else
            {
                bool hasSerializeMethod = (ReferenceTypeSerializeMethods.GetMethodForType(type) != null);
                Debug.Assert(hasSerializeMethod == (ReferenceTypeDeserializeMethods.GetMethodForType(type) != null)); // <- debug check for matching deserializer
                return hasSerializeMethod;
            }
        }
        ...
        public bool HasFieldSerializer(Type type)
        {
            ...
            {
                bool hasSerializeMethod = (ReferenceFieldSerializeMethods.GetMethodForType(type) != null);
                Debug.Assert(hasSerializeMethod == (ReferenceFieldDeserializeMethods.GetMethodForType(type) != null)); // <- debug check for matching deserializer
                return hasSerializeMethod;
            }
        }
