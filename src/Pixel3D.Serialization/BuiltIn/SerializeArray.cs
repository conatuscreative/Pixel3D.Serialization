...
namespace Pixel3D.Serialization.BuiltIn
{
    static class SerializeArray
    {
        ...
        public static void SerializeArray2DField<T>(SerializeContext context, BinaryWriter bw, T[,] array)
        {
            ...
            {
                for(int j = 0; j < length1; j++) // NOTE: Arrays are contiguious on their rightmost (last) dimension
                {
        ...
        internal class ArrayFieldMethodProvider : MethodProvider
        {
            ...
            MethodInfo[] arraySerializeMethods;
            ...
            public override MethodInfo GetMethodForType(Type type)
            {
                ...
                {
                    int lookup = type.GetArrayRank() - 1;
                    if(lookup < 0 || lookup >= arraySerializeMethods.Length)
                    {
                        Debug.Assert(false); // <- Trying to serialize a multidimensional array we don't support (add it? add arbitrary dimension support?)
                        return null;
                    }
