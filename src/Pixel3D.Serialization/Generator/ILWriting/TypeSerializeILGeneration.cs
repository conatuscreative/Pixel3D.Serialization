...
{
    static class TypeSerializeILGeneration
    {
        //
        ...
        static readonly string HAS_VALUE_FIELD = GetHasValueField();
        static string GetHasValueField()
        {
        ...
        public static void GenerateReferenceTypeSerializationMethod(Type type, ILGenerator il, ILGenContext context)
        {
            ...
            if(context.Serialize) // Serialize visits and leaves objects like a stack (including up the inheritance hierarchy within a single object instance)
                GenerateReferenceTypeVisitObject(type, il, context);

            MethodInfo baseTypeSerializer = context.referenceTypeSerializationMethods[type.BaseType];
            ...
            else if(!context.Serialize) // Deserialize visits objects once only to get their reference (so only visit the base)
            {
                GenerateReferenceTypeVisitObject(type, il, context);
            }
        ...
        private static void GenerateReferenceTypeVisitObject(Type type, ILGenerator il, ILGenContext context)
        {
            il.Emit(OpCodes.Ldarg_0);
            il.Emit(OpCodes.Ldarg_2);
            il.Emit(OpCodes.Callvirt, context.Serialize ? Methods.SerializeContext_VisitObject : Methods.DeserializeContext_VisitObject);
        }
        ...
        private static void GenerateDebugTrace(string traceString, ILGenerator il, ILGenContext context)
        {
            ...
            il.Emit(OpCodes.Callvirt, context.Serialize ? Methods.SerializeContext_DebugTrace : Methods.DeserializeContext_DebugTrace);
#endif
        }
        ...
        private static void GenerateNullableTypeSerializeMethod(Type type, ILGenerator il, ILGenContext context)
        {
            ...
            {
                // obj.value = default(Struct)  // <- Maintain Nullable internal invariant (GetValueOrDefault() depends on this!)
        ...
        private static void GenerateSerializeAllTypeFields(Type type, ILGenerator il, ILGenContext context)
        {
            ...
            {
                int bytesRequired = (boolFields.Length+7) / 8;

                if(context.Serialize)
                {
                    for(int i = 0; i < bytesRequired; i++)
                    {
                        int bitsRequired = Math.Min(8, boolFields.Length - i*8);
                        Debug.Assert(bitsRequired > 0);

#if DEBUG
                        string traceString = "Boolean fields (" + i + "): " + string.Join(", ", boolFields.Select(f => f.Name).ToArray(), (i*8), bitsRequired);
            ...
            //       -AR
            var nonBoolFields = type.GetFields(TypeDiscovery.allInstanceDeclared).Where(f => f.FieldType != typeof(bool)).NetworkOrder(f => f.Name);      
        ...
        private static void GenerateSerializePrimitiveField(Type type, FieldInfo field, ILGenerator il, ILGenContext context)
        {
            if(context.Serialize)
            {
                il.Emit(OpCodes.Ldarg_1); // bw
        ...
        private static void GenerateSerializeValueTypeField(Type type, FieldInfo field, ILGenerator il, ILGenContext context)
        {
        ...
        private static void GenerateSerializeReferenceTypeField(Type type, FieldInfo field, ILGenerator il, ILGenContext context)
        {
            Debug.Assert(!field.FieldType.IsValueType);
            il.Emit(OpCodes.Ldarg_0); // context
            il.Emit(OpCodes.Ldarg_1); // bw
            il.Emit(OpCodes.Ldarg_2); // obj
            il.Emit(context.Serialize ? OpCodes.Ldfld : OpCodes.Ldflda, field); // obj.'field' (byref for deserialize only!)
            il.Emit(OpCodes.Call, context.fieldSerializationMethods[field.FieldType]);
        }
