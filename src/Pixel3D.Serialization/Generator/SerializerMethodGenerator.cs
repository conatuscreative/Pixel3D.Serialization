...
namespace Pixel3D.Serialization.Generator
{
    ...
    internal class SerializerMethodGenerator
    {
        ...
        IEnumerable<Type> suplementalDispatchTypes; // can be null

        public SerializerMethodGenerator(TypeClassifier typeClassifier, SerializationMethodProviders customMethods, IEnumerable<Type> suplementalDispatchTypes)
        {
        ...
        MethodProvider fieldSerializeMethods;
        MethodProvider fieldDeserializeMethods;
        ...
        MethodInfo deserializationDispatcherMethod;
        ...
        public GeneratorResult Generate(MethodCreatorCreator methodCreatorCreator)
        {
            ...
            MethodCreator valueTypeSerializerType = methodCreatorCreator.Create("ValueTypeSerializer");
            MethodCreator valueTypeDeserializerType = methodCreatorCreator.Create("ValueTypeDeserializer");

            Dictionary<Type, MethodInfo> generatedValueTypeSerializeMethods = new Dictionary<Type, MethodInfo>();
            Dictionary<Type, MethodInfo> generatedValueTypeDeserializeMethods = new Dictionary<Type, MethodInfo>();
            ...
            foreach(var valueType in typeClassifier.ValueTypes)
            {
                ...
                var serializeMethod = valueTypeSerializerType.CreateMethod(valueType, "ValueTypeSerialize", typeof(void), new[] { typeof(SerializeContext), typeof(BinaryWriter), valueType.MakeByRefType() });
                generatedValueTypeSerializeMethods.Add(valueType, serializeMethod);

                var deserializeMethod = valueTypeDeserializerType.CreateMethod(valueType, "ValueTypeDeserialize", typeof(void), new[] { typeof(DeserializeContext), typeof(BinaryReader), valueType.MakeByRefType() });
            ...
            MethodCreator referenceTypeSerializerType = methodCreatorCreator.Create("ReferenceTypeSerializer");
            MethodCreator referenceTypeDeserializerType = methodCreatorCreator.Create("ReferenceTypeDeserializer");

            Dictionary<Type, MethodInfo> generatedReferenceTypeSerializeMethods = new Dictionary<Type, MethodInfo>();
            Dictionary<Type, MethodInfo> generatedReferenceTypeDeserializeMethods = new Dictionary<Type, MethodInfo>();
            ...
            foreach(var referenceType in typeClassifier.ReferenceTypes)
            {
                ...
                var serializeMethod = referenceTypeSerializerType.CreateMethod(referenceType, "ReferenceTypeSerialize", typeof(void), new[] { typeof(SerializeContext), typeof(BinaryWriter), referenceType });
                generatedReferenceTypeSerializeMethods.Add(referenceType, serializeMethod);

                var deserializeMethod = referenceTypeDeserializerType.CreateMethod(referenceType, "ReferenceTypeDeserialize", typeof(void), new[] { typeof(DeserializeContext), typeof(BinaryReader), referenceType });
            ...
            MethodCreator fieldSerializerType = methodCreatorCreator.Create("FieldSerializer");
            MethodCreator fieldDeserializerType = methodCreatorCreator.Create("FieldDeserializer");

            Dictionary<Type, MethodInfo> generatedReferenceFieldSerializeMethods = new Dictionary<Type, MethodInfo>();
            Dictionary<Type, MethodInfo> generatedReferenceFieldDeserializeMethods = new Dictionary<Type, MethodInfo>();
            ...
            foreach(var fieldType in typeClassifier.fieldSerializerDispatchLookup.Keys)
            {
                ...
                var serializeMethod = fieldSerializerType.CreateMethod(fieldType, "FieldSerialize", typeof(void), new[] { typeof(SerializeContext), typeof(BinaryWriter), fieldType });
                generatedReferenceFieldSerializeMethods.Add(fieldType, serializeMethod);

                var deserializeMethod = fieldDeserializerType.CreateMethod(fieldType, "FieldDeserialize", typeof(void), new[] { typeof(DeserializeContext), typeof(BinaryReader), fieldType.MakeByRefType() });
            ...
            MethodCreator serializerDispatchType = methodCreatorCreator.Create("SerializerDispatch");
            MethodCreator deserializerDispatchType = methodCreatorCreator.Create("DeserializerDispatch");

            // These are dispatched by ID#, so this is a lookup by that ID#
            List<SerializationMethods> dynamicDispatchMethods = new List<SerializationMethods>(typeClassifier.dispatchableTypes.Count);
            ...
            {
                string idString = dynamicDispatchMethods.Count.ToString("0000");
                SerializationMethods sm;
                sm.type = type;

                sm.serializer = serializerDispatchType.CreateMethod(type, "SerializeWithId_" + idString, typeof(void), new[] { typeof(SerializeContext), typeof(BinaryWriter), typeof(object) });
                sm.deserializer = deserializerDispatchType.CreateMethod(type, "DeserializeFromId_" + idString, type, new[] { typeof(DeserializeContext), typeof(BinaryReader) });
            ...
            deserializationDispatcherMethod = deserializerDispatchType.CreateMethod(null, "DeserializationDispatcher", typeof(object), new[] { typeof(DeserializeContext), typeof(BinaryReader) });
            ...
            fieldSerializeMethods = FallbackMethodProvider.Combine(new MethodProvider[] {
                    customMethods.ValueTypeSerializeMethods,
            ...
            ILGenContext serializeContext = new ILGenContext(Direction.Serialize, fieldSerializeMethods, referenceTypeSerializeMethods);
            ILGenContext deserializeContext = new ILGenContext(Direction.Deserialize, fieldDeserializeMethods, referenceTypeDeserializeMethods);
            ...
            for(int i = 0; i < dynamicDispatchMethods.Count; i++)
            {
                // SerializeWithId Method:
                #region Generate IL
                {
                    ILGenerator il = dynamicDispatchMethods[i].serializer.GetILGenerator();
                ...
                #region Generate IL
                {
                    ILGenerator il = dynamicDispatchMethods[i].deserializer.GetILGenerator();
            ...
            foreach(var sm in generatedValueTypeSerializeMethods)
                TypeSerializeILGeneration.GenerateValueTypeSerializationMethod(sm.Key, sm.Value.GetILGenerator(), serializeContext);
            foreach(var sm in generatedValueTypeDeserializeMethods)
                TypeSerializeILGeneration.GenerateValueTypeSerializationMethod(sm.Key, sm.Value.GetILGenerator(), deserializeContext);
            foreach(var sm in generatedReferenceTypeSerializeMethods)
                TypeSerializeILGeneration.GenerateReferenceTypeSerializationMethod(sm.Key, sm.Value.GetILGenerator(), serializeContext);
            foreach(var sm in generatedReferenceTypeDeserializeMethods)
                TypeSerializeILGeneration.GenerateReferenceTypeSerializationMethod(sm.Key, sm.Value.GetILGenerator(), deserializeContext);
            ...
            {
                ILGenerator il = deserializationDispatcherMethod.GetILGenerator();

                Label[] dispatchLabels = new Label[dynamicDispatchMethods.Count];
                ...
                // br.ReadInt32()
                il.Emit(OpCodes.Ldarg_1);
                il.Emit(OpCodes.Callvirt, typeof(BinaryReader).GetMethod("ReadInt32")); // NOTE: C# compiler loads this into two locals for no reason

                // switch(...)
                il.Emit(OpCodes.Switch, dispatchLabels);
                il.Emit(OpCodes.Br, failLabel); // default case // NOTE: C# compiler emits br.s, but I bet it's not short with our huge table!
            ...
            Dictionary<Type, SerializeDispatchDelegate> dispatchTable = new Dictionary<Type, SerializeDispatchDelegate>();
            ...
            if(methodCreatorCreator is DynamicMethodCreatorCreator)
            {
                for(int i = 0; i < dynamicDispatchMethods.Count; i++)
                {
                    DynamicMethod serializerMethod = dynamicDispatchMethods[i].serializer as DynamicMethod;
                    Debug.Assert(serializerMethod != null);
                    var serializerDelegate = (SerializeDispatchDelegate)serializerMethod.CreateDelegate(typeof(SerializeDispatchDelegate));
            ...
            SerializationMethodProviders generatedMethodProviders = new SerializationMethodProviders(
                    new LookupMethodProvider(generatedValueTypeSerializeMethods),
                    new LookupMethodProvider(generatedValueTypeDeserializeMethods),
                    new LookupMethodProvider(generatedReferenceTypeSerializeMethods),
                    new LookupMethodProvider(generatedReferenceTypeDeserializeMethods),
            ...
            SerializationMethodProviders combinedMethodProviders = SerializationMethodProviders
                    .Combine(customMethods, generatedMethodProviders);


            DeserializeDispatchDelegate deserializeDispatchDelegate = null;
            if(methodCreatorCreator is DynamicMethodCreatorCreator) // Only dynamic methods can be converted to delegates (our dynamic assembly is save-only)
            {
                deserializeDispatchDelegate = (DeserializeDispatchDelegate)(deserializationDispatcherMethod as DynamicMethod).CreateDelegate(typeof(DeserializeDispatchDelegate));
            }
            

            return new GeneratorResult(combinedMethodProviders, dispatchTable, deserializeDispatchDelegate, null); // module table gets filled later
        ...
        private void GenerateReferenceFieldSerializer(ILGenerator il, Type fieldType, HashSet<Type> dispatchToTypes)
        {
            ...
            if(dispatchToTypes.Count == 0)
            {
                ...
                MethodInfo serializeMethod = referenceTypeSerializeMethods[fieldType];
            ...
            else if(dispatchToTypes.Count == 1)
            {
                ...
                Type directDispatchType = dispatchToTypes.First();
                Type castDown = directDispatchType.IsAssignableFrom(fieldType) ? null : directDispatchType;
                GenerateReferenceFieldSerializerDispatchCall(il, referenceTypeSerializeMethods[directDispatchType], castDown);
            }
            else // Multiple dispatch targets
            {
                // Convert CLR type to type ID via lookup table (writes type ID and dispatches)
                GenerateReferenceFieldSerializerDispatchCall(il, Methods.StaticDispatchTable_SerializationDispatcher, null);
            }
        }


        void GenerateReferenceFieldSerializerWalk(ILGenerator il, Type fieldType)
        {
        ...
        void GenerateReferenceFieldSerializerDispatchCall(ILGenerator il, MethodInfo targetMethod, Type castDown = null)
        {
            il.Emit(OpCodes.Ldarg_0);
            il.Emit(OpCodes.Ldarg_1);
            il.Emit(OpCodes.Ldarg_2);
        ...
        private void GenerateReferenceFieldDeserializer(ILGenerator il, Type fieldType, HashSet<Type> dispatchToTypes)
        {
            ...
            il.Emit(OpCodes.Ldarg_0);
            il.Emit(OpCodes.Ldarg_2);
            il.Emit(OpCodes.Callvirt, typeof(DeserializeContext).GetMethod("Walk").MakeGenericMethod(new[] { fieldType }));
            il.Emit(OpCodes.Brtrue_S, checkPassLabel);
            ...
            else if(dispatchToTypes.Count == 1) // One dispatch target: Do direct dispatch
            {
                ...
                if(!directDispatchType.IsAssignableFrom(fieldType)) // Direct dispatch where the field is an abstract type, dispatching to a single concrete type (requires a cast)
            ...
            else // Multiple dispatch targets: Do dynamic dispatch
            {
                ...
                il.Emit(OpCodes.Ldarg_0);
                il.Emit(OpCodes.Ldarg_1);
                il.Emit(OpCodes.Call, deserializationDispatcherMethod); // <- will read object type ID and dispatch on it (will create and deserialize the object)
        ...
        private void GenerateTypeCreation(ILGenerator il, Type type)
        {
            ...
            if(customInitializeMethod != null)
            {
                ...
                if(type != customInitializeMethod.ReturnType)
                    throw new Exception("Internal Error: Bad custom initializer return type"); // Stick a cast class in here if this ever becomes possible
